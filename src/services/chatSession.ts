    ...
    messages:
      preloadedMessages && preloadedMessages.length > 0
        ? [
            ...
            ...preloadedMessages.filter((m) => m.role !== 'system'),
          ]
        : [{ role: 'system', content: systemPrompt }],
...
function createChatContext(
  ...
  return {
    ...
    get thinkingSupported() {
      return state.thinkingSupported;
    },
...
function applyEffectiveNumCtx(state: ChatSessionState): void {
  state.numCtx =
    state.modelContextLimit && state.modelContextLimit > 0
...
export async function loadModelMetadata(state: ChatSessionState, config: Config): Promise<void> {
...
export function getCurrentTokenEstimate(state: ChatSessionState): number {
  ...
  if (
    state.messages.length !== state.cachedMessagesLength ||
  ...
  if (state.lastAuthoritativeTokens > 0 && state.estimatedTokensAtAuthoritative > 0) {
    return Math.max(
      0,
      state.lastAuthoritativeTokens +
        (state.cachedTokenTotal - state.estimatedTokensAtAuthoritative)
    );
  }
...
export function refreshTokenStatus(
  ...
  if (state.numCtx > 0) {
    ...
    if (
      percentage >= COMPACT_WARNING_THRESHOLD_PCT &&
...
export async function autoCompactIfNeeded(
  ...
  try {
    ...
    const result = await compactHistory(
      ...
      state.numCtx,
      (status: string) =>
        refreshTokenStatus(state, status, undefined, 'estimated', compactionModel),
      1,
      2,
      (stats) => {
