...
export async function dispatchMCPToolCall(
  ...
  if (!parsed) {
    return {
      content: `[MCP error: tool name "${namespacedName}" is not a valid mcp__<server>__<tool> name]`,
    };
  }
  ...
  if (handle.status !== 'connected') {
    return {
      content: `[MCP error: server "${serverName}" is in status "${handle.status}"${handle.lastError ? `: ${handle.lastError}` : ''}]`,
    };
  }
  ...
  if (!tool) {
    return {
      content: `[MCP error: tool "${toolName}" is not available on server "${serverName}"]`,
    };
  }
  ...
  const approvedByAutoApprove =
    handle.config.autoApprove?.some(
      (pattern) =>
  ...
  if (result.kind === 'aborted') {
    return {
      content: `[MCP error: call to "${serverName}/${toolName}" was aborted by the client]`,
    };
  }
  if (result.kind === 'timeout') {
    return {
      content: `[MCP error: call to "${serverName}/${toolName}" timed out after ${timeoutMs}ms]`,
    };
  }
...
function formatMCPResult(
  ...
  for (const block of conventional.content) {
    ...
    if (block.type === 'audio') {
      textBlocks.push(
        `[audio block dropped: ${block.mimeType} — audio content is not yet supported in Locopilot]`
      );
      continue;
    }
...
export function getApprovedNamespacedNames(configured: Set<string>): string[] {
  return [...configured].filter((name) => parseMCPToolName(name) !== null);
}
