...
export const mcpCallToolSchema: ToolSchema = {
    name: 'mcp_call',
    description: 'Invoke a tool exposed by a configured MCP (Model Context Protocol) server. ' +
    ...
    parameters: {
        type: 'object',
        properties: {
            server:     { type: 'string', description: 'The name of the MCP server (the key from `mcpServers` in ~/.locopilot/mcp.json). Must be kebab-case, 1-64 characters.' },
            tool:       { type: 'string', description: 'The bare tool name as reported by the MCP server (e.g. "list_issues"). Do NOT include the `mcp__<server>__` prefix here; the system adds that automatically.' },
            arguments:  { type: 'object', description: "Optional JSON object of arguments to pass to the MCP tool. Shape is server-defined; see the tool's schema in the system tool list." },
        },
...
export async function runMCPCall(args: ToolCallArguments, context?: RequestContext, signal?: AbortSignal): Promise<ToolCallResult> {
    ...
    const disabledSubAgent = context?.subAgent && Array.isArray((context.subAgent as { disabledTools?: string[] }).disabledTools)
    ...
    if (disabledMain.includes(namespacedName) || disabledMain.includes('mcp_call') ||
        disabledSubAgent.includes(namespacedName) || disabledSubAgent.includes('mcp_call')) {
        return {
    ...
    if (context?.allowedTools && context.allowedTools.length > 0) {
        if (!context.allowedTools.includes(namespacedName) &&
            !context.allowedTools.includes('mcp_call')) {
            return {
                content: `[Error: tool "${namespacedName}" is not allowed by the currently active skills. ` +
    ...
    const approvedTools: Set<string> | undefined =
        context?.mcpApprovals ? new Set(context.mcpApprovals) : undefined;
