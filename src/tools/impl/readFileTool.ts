...
export const readFileToolSchema: ToolSchema = {
    name: 'read_file',
    description: 'Reads a file from the host filesystem. Use head_chars to read the first N characters, tail_chars to read the last N characters, start/length to read a specific character range, or start_line/end_line to read by line number (1-based). Supports absolute paths, relative paths (resolved against the agent working directory), and ~/ paths.',
    parameters: {
        type: 'object',
        properties: {
            ...
            start_line:  { type: 'number', description: '1-based line number at which to begin reading. Use with end_line or line_count.' },
            end_line:    { type: 'number', description: '1-based line number at which to end reading (inclusive). Defaults to the last line if omitted.' },
            line_count:  { type: 'number', description: 'Number of lines to read starting at start_line. Only valid when start_line is also provided.' },
        },
        required: ['path'],
    },
};
...
export class ReadFileTool {
    ...
    async run(args: ReadFileToolArgs, signal?: AbortSignal): Promise<string> {
        ...
        const isLineRange = start_line !== undefined || end_line !== undefined || line_count !== undefined;

        const activeModes = [isCharRange, isHeadTail, isLineRange].filter(Boolean).length;
        if (activeModes > 1) {
            const errorMsg = '[read_file error: specify only one reading mode — character range (start/length), head/tail (head_chars/tail_chars), or line range (start_line/end_line/line_count).]';
            this.log(errorMsg);
            return errorMsg;
        }
        ...
        if (line_count !== undefined) {
            ...
            if (start_line === undefined) {
                const errorMsg = '[read_file error: "line_count" requires "start_line" to also be provided.]';
                this.log(errorMsg);
                return errorMsg;
            }
        }
        ...
        const effectiveEndLine = end_line ?? (start_line !== undefined && line_count !== undefined ? start_line + line_count - 1 : undefined);
        if (start_line !== undefined && effectiveEndLine !== undefined && effectiveEndLine < start_line) {
        ...
        try {
            ...
            if (isLineRange) {
                ...
                const endIdx = effectiveEndLine !== undefined
                    ? Math.min(effectiveEndLine, lines.length)
                    : lines.length;
