  ...
  parameters: {
    ...
    properties: {
      path: {
        type: 'string',
        description:
          'Path to the PDF file, absolute or relative to the agent working directory. Supports ~/ paths.',
      },
      start_page: {
        type: 'number',
      ...
      end_page: {
        type: 'number',
        description:
          'Last page to read (inclusive). Defaults to start_page + 49 (max 50 pages per call).',
      },
      extract_images: {
        type: 'boolean',
...
export class ReadPdfTool {
  ...
  async run(args: ReadPdfToolArgs, signal?: AbortSignal): Promise<ToolCallResult> {
    ...
    try {
      ...
      return {
        content: `[read_pdf error: unable to access file: ${err instanceof Error ? err.message : String(err)}]`,
      };
    }
    ...
    try {
      ...
      return {
        content: `[read_pdf error: failed to read file: ${err instanceof Error ? err.message : String(err)}]`,
      };
    }
    ...
    try {
      ...
      return {
        content: `[read_pdf error: failed to parse PDF: ${err instanceof Error ? err.message : String(err)}]`,
      };
    }
    ...
    try {
      ...
      if (startPage > totalPages) {
        return {
          content: `[read_pdf error: start_page ${startPage} exceeds total pages (${totalPages}).]`,
        };
      }

      if (startPage > endPage) {
        return {
          content: `[read_pdf error: start_page (${startPage}) cannot be greater than end_page (${endPage}).]`,
        };
      }
      ...
      try {
        ...
        return {
          content: `[read_pdf error: failed to extract text: ${err instanceof Error ? err.message : String(err)}]`,
        };
      }
      ...
      if (this.model && this.numCtx && extractedText.length > 0) {
        ...
        extractedText.length >= READ_FILE_CHAR_WARN_THRESHOLD
      ) {
      ...
      if (args.extract_images) {
        try {
          ...
          this.log(
            `read_pdf: image extraction failed: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      }
      ...
      return images !== undefined && images.length > 0 ? { content, images } : { content };
    } finally {
      try {
        await parser.destroy();
      } catch {
...
export function getToolPrompt(): string {
  ...
  return `12. ${s.name}(path, start_page?, end_page?, extract_images?)\n` + `   ${s.description}\n`;
}
