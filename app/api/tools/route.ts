...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    ...
    const toolsConfig = { disabledMain: [...(config.tools?.disabledMain ?? [])], disabledSubAgent: [...(config.tools?.disabledSubAgent ?? [])] };
    ...
    return NextResponse.json({ tools: buildToolList(toolsConfig.disabledMain, toolsConfig.disabledSubAgent) });
}
