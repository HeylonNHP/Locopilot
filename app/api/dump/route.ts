...
import { buildConversationDumpMarkdown, buildDumpFileName, type ConversationDumpInput } from '../../../services/historyDump';
...
export async function POST(request: NextRequest): Promise<Response> {
    ...
    try {
        ...
        const effectiveBaseUrl = typeof body.baseUrl === 'string' && body.baseUrl.trim().length > 0
            ? body.baseUrl.trim()
            : config?.baseUrl?.trim() || 'http://localhost:11434';
        const effectiveNumCtx = parsePositiveInteger(body.numCtx)
            ?? (config?.numCtx && Number.isFinite(config.numCtx) && config.numCtx > 0
                ? Math.trunc(config.numCtx)
                : DEFAULT_NUM_CTX);
        const savedNumCtx = config?.numCtx && Number.isFinite(config.numCtx) && config.numCtx > 0
            ? Math.trunc(config.numCtx)
            : undefined;
