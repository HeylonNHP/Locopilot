...
export const fetchUrlToolSchema: ToolSchema = {
    ...
    parameters: {
        ...
        properties: {
            ...
            full_content: { type: 'boolean', description: 'When true, returns the full extracted page text without applying the per-page character limit compaction. Useful when you need complete content such as large code snippets, documentation pages, or detailed articles that should not be truncated. Defaults to false (compaction applies when content exceeds the configured limit). When the returned content is large relative to your context window, a warning will be appended to the result.' },
        },
        required: ['url'],
    },
};
...
export class FetchUrlTool {
    ...
    async run(args: FetchUrlToolArgs, signal?: AbortSignal): Promise<string> {
        ...
        try {
            ...
            const tokenWarning = buildContextWindowWarning(result.text, this.model, this.numCtx, args.full_content === true);
