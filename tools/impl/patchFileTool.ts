...
function formatCurrentExcerpt(content: string, patch: Partial<PatchFilePatch>): string {
    ...
    const lines = [`closest current excerpt (line ${preview.startLine + 1}, score ${preview.score}):`, '@@'];
...
function formatPatchError(
    ...
    const lines = [`[patch_file error: patch ${patchIndex + 1} of ${patchCount} ${reason}]`, `path: ${path}`];
...
function resolvePatchMatch(content: string, patch: PatchFilePatch): MatchRange | { error: string; matches?: MatchRange[] } {
    ...
    if (normalizedMatches.length > 1) {
        return { error: 'matched multiple locations after whitespace normalization', matches: normalizedMatches };
    }
...
export class PatchFileTool {
    ...
    private log(message: string): void {
        this.output.writeLine(`[PatchFileTool] ${message}`);
    }
