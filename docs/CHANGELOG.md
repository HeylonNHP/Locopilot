- 2026-09-11: Sampling-parameter cache now applies uniformly to every caller of the openai-compatible adapter, not just the main chat turn
  - Files: `src/services/samplingParamsCache.ts`, `src/services/adapters/openaiCompatibleAdapter.ts`, `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: The unsupported-sampling-param cache (e.g. "this model rejects `temperature`") was only ever populated onto `params.samplingParamSupport` by the main chat route, and only that route's 400 catch block recorded a fresh discovery. Every other caller of the adapter — the prompt-loop judge/critic (`src/services/promptLoop.ts`), sub-agents (`src/tools/impl/subAgentTool.ts`), compaction, and title generation — builds its own request via `options.*` and never resolves or threads the cache through, so a model already known (from an earlier 400, in the same session) to reject a param kept getting it re-sent, and re-rejected, forever from those call sites. Added `peekSamplingParamSupportMap` (synchronous, no probe, no network) and `isKnownSamplingParam` to `samplingParamsCache.ts`; `openaiCompatibleAdapter.ts`'s `sendOpenAICompatibleChat`/`sendOpenAICompatibleChatStream` now resolve an effective support map for every request (caller-supplied map if present, else a cache peek) before building the payload, and their own catch blocks parse every failure for an unsupported-param signal and record it immediately via `recordDiscoveredUnsupportedParam` — regardless of which caller made the request. The chat route's own resolve/record logic is unchanged (it still drives the proactive per-turn probe and the client-facing SSE `sampling_param_unsupported` event); the adapter-level path is what makes the _reactive_ half apply everywhere else too.
  - Intent: A parameter discovered unsupported for a model should be silently dropped on every subsequent call to that model, and any failing call — not just the main chat turn — should teach the cache, so the fix does not depend on which code path happens to hit the 400 first.
  - Verification: `tsc --noEmit` clean, `eslint` clean on both changed files, existing `test-sampling-params-cache.mjs` (55 checks) unchanged and passing, plus an ad hoc check confirming a discovery recorded via the adapter's exact error-parsing path is visible to an unrelated caller doing a bare cache peek.
- 2026-09-10: Remove display-only sub-agent prefixes from web UI logs
  - Files: `src/tools/toolOutput.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/api/chat/route.ts`, `src/components/ChatMessageBubble/SubagentLogBubble/SubagentLogBubble.tsx`, `scripts/test-subagent-output.mjs` (new), `package.json`, `CLAUDE.md`, `WEBUI_MIGRATION.md`, `docs/CHANGELOG.md`
  - Summary: Sub-agent output used to embed a display-only agent label in every line, then the web route parsed that text back into the structured agent ID used for SSE routing and persistence. The new optional `ToolOutputSink.writeAgentLine(agentId, message)` path keeps attribution as metadata, so log text is clean and each agent still receives its own bubble. Approval/request lines also no longer contain a redundant agent label; the existing `agentId`, `subagentId`, `subagent_id`, SSE events, and persistence behavior are unchanged. The bubble heading now shows the agent name without the redundant `Sub-agent:` label. Unstructured sink writes retain the defensive `__subagent__` fallback.
  - Verification: added `test:subagent-output` covering structured IDs, multiline output, absence of textual prefixes, and plain-text fallback behavior; type-check, lint, formatting, and the existing sub-agent/MCP regression suite pass.
- 2026-09-09: Stronger default-on guidance for shared sub-agent summaries
  - Files: `src/tools/impl/subAgentTool.ts`, `src/tools/tools.ts`, `scripts/test-subagent-shared-summaries.mjs` (new), `package.json`, `docs/CHANGELOG.md`
  - Summary: `run_subagents` already shared completed summaries unless the caller explicitly passed `share_summaries: false`, but the preference was easy for the orchestrating LLM to miss. Added one shared guidance constant used by the tool schema and generated tool prompt, plus an explicit global tool-policy bullet. The guidance recommends omitting `share_summaries` or setting it to `true` for dependent workflows such as research → implementation → review, so later agents build on completed work rather than repeating it. It preserves `false` for genuinely independent tasks such as blind reviews, parallel alternatives, or work where prior conclusions could cause anchoring or cross-contamination. Runtime semantics are unchanged; only the instruction surfaces are stronger.
  - Verification: added `test:subagent-shared-summaries` (5 checks passed); `tsc`, ESLint, Prettier, and the existing sub-agent/MCP regression suite pass.
- 2026-09-08: Collapsible provider sections in the model selector
  - Files: `src/components/ModelSelector/ModelSelector.tsx`, `src/components/ModelSelector/collapsedProviders.ts` (new), `src/components/ModelSelector/ModelSelector.scss`, `docs/CHANGELOG.md`
  - Summary: Each provider section in the model selector dropdown (main model + compaction mode) is now collapsible via its header. Design: collapse state lives in a small module-scoped external store consumed with `useSyncExternalStore` (`collapsedProviders.ts`) — NOT chatStore, because it is ephemeral view state and a dispatch per header click would re-render the whole app tree mid-stream; the external store also shares one state between the two simultaneously mounted ModelSelector instances and persists to localStorage (`locopilot.modelSelector.collapsedProviders`, validated on read, absence = expanded) so choices survive reloads. Search overrides collapse: while a search term is active every matching model is shown and headers render as inert labels (a toggle with no visible effect would be a dead affordance). Headers are native buttons with `aria-expanded`/`aria-controls` (`useId`), a rotating chevron, a per-provider model count, a visible `:focus-visible` ring, and a small dot on collapsed sections that contain the active model so the current selection is never lost. The render path was decomposed from a `.flatMap([header, ...items])` JSX block into `ProviderSection` + `ModelItem` components with an order-preserving `groupModelsByProvider` Map helper. One Sass/Turbopack gotcha encoded in the SCSS: trailing parent selectors (`button&`) are rejected by the build ("& may only used at the beginning of a compound selector"), so the button-only styles use `&:is(button)`.
  - Intent: Let users with many providers/models collapse the sections they don't use, without losing track of the active model or breaking search.
  - Verification: `tsc` clean, `eslint` clean, production build passes (after fixing the two Sass selector syntax errors it exposed), and all script suites unchanged (the one `test-provider-persistence` failure, "reads providerId from the matched model", is pre-existing on the parent commit and unrelated to this change).
- 2026-09-07: Live t/s badge during conversation compaction - the t/s badge keeps updating while compaction runs
  - Files: `src/services/compact/orchestrate.ts`, `src/services/compact/summarize.ts`, `src/services/compact/reduce.ts`, `src/services/compact/distill.ts`, `src/app/api/chat/route.ts`, `src/app/api/compact/route.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/hooks/useSlashCommands.ts`, `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: While a conversation compaction ran, the t/s badge went stale even though compaction involves many long LLM calls (tool-output distillation, single-shot or map-reduce summarisation). `compactHistory` now accepts an optional `onTps` reporter threaded through the distill, summarise, chunk and reduction steps; each LLM call drives a shared-policy `LiveThroughputMeter` (rebaseline at first token, min interval, minimum measurement window). The chat route's auto-compact and the `/compact` slash command emit it as `status { phase: 'compacting', tps }`, sub-agent auto-compaction forwards it via `output.reportTps`, and the `/compact` SSE loop in `useSlashCommands` consumes it, clearing the badge when compaction ends. The token-measurement call is deliberately not wired (it generates at most one token, so it has no meaningful rate).
  - Intent: Give the user live feedback about compaction speed on the same t/s badge used for generation, without contaminating the authoritative end-of-turn stats.
- 2026-09-07: Fixed six-figure t/s spikes from bulk token arrivals measured over tiny windows
  - Files: `src/services/tokenThroughput.ts`, `src/app/api/chat/route.ts`, `src/tools/impl/subAgentTool.ts`, `scripts/test-token-throughput.mjs`, `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: After the t/s refactor, the badge intermittently displayed absurd rates (observed: 236,666.67 t/s and 639,000.00 t/s) that "often correct themselves". Root cause was the retroactive tool-call-argument counting introduced in the same refactor: a tool-call-only LLM response (exactly what command-running sub-agents produce) surfaces its entire argument payload - often thousands of tokens of run_command JSON - in one chunk. The `LiveThroughputMeter` counted that JSON into the numerator at arrival time and rebaselined its clock at that same instant (it was the first token-bearing chunk), so the next chunk - the done chunk, a few milliseconds later - passed the emission gates and computed e.g. 5000 tokens / 3 ms = 1.6M t/s (observed values were milder variants like 2000/3ms). The freeze semantics then pinned the absurd value on the badge for the whole tool execution, until the next LLM call's first reports corrected it - matching both screenshots (huge values frozen while sub-agents ran shell commands). Two-part fix: (1) removed the retroactive argument counting entirely - argument JSON surfaces in one chunk with no honest timestamp, so it has no place in a streaming rate; the authoritative end-of-turn rate from the provider's eval counts (TurnThroughputAggregator) already covers those tokens (both consumers' `onJson` calls removed); (2) the meter now requires at least MIN_REPORT_WINDOW_MS (1 s) of measured generation time before emitting, and arrivals skipped for a too-small window are acknowledged so they can never resurface later as a diluted spike - the badge instead keeps its last honest value until genuinely new tokens arrive. This also bounds the related buffered-flush case (a provider dumping thousands of buffered stream tokens in one chunk early in the measurement).
  - Intent: The live t/s badge must never divide a bulk token arrival by a few-millisecond window; a rate that cannot be measured honestly is not shown at all.
  - Covered by 49 tests in `scripts/test-token-throughput.mjs` (new regression tests: sub-second windows are never reported, bulk arrivals never resurface as diluted spikes, bounded emission once the window reaches one second); all other suites unchanged and passing.

---

- 2026-09-07: Fixed six tokens-per-second read-out bugs and extracted the t/s policy into shared modules
  - Files: `src/services/tokenThroughput.ts` (new), `src/app/lib/tpsDisplay.ts` (new), `src/app/api/chat/route.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/hooks/useChatStream.ts`, `src/app/lib/chatStore.ts`, `src/components/StatusBar/StatusBar.tsx`, `src/types/sse.ts`, `scripts/test-token-throughput.mjs` (new), `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: A subagent audit of the status bar's t/s read-out confirmed six display bugs plus several latent fragilities, all fixed here. (1) The live rate divided streamed tokens by wall time starting BEFORE the HTTP request, so it included connection setup, model load, and prompt processing - a 10 s load followed by 5 s at 60 t/s showed ~20 t/s live, then jumped to ~60 t/s at turn end. The new `LiveThroughputMeter` rebaselines its clock at the first token-bearing chunk. (2) The live numerator only counted thinking/content chunks, so while the model streamed a large tool-call argument the numerator froze while the denominator grew - the badge decayed toward ~0 t/s during exactly the heaviest generation. The meter now only emits when new tokens arrived (the last rate freezes instead of decaying) and tool-call argument JSON is counted retroactively from the chunk that surfaces it. (3) Counts/durations were overwritten by every `chunk.done` and durations were re-declared per tool-loop iteration, so the `done` t/s described only the last (shortest, noisiest) LLM call. The new `TurnThroughputAggregator` aggregates every call in the turn as sum(evalCount)/sum(evalDuration), with a per-call wall-clock fallback (measured from first token, so load/prompt-eval excluded) for providers that report no durations. (4) OpenAI-compatible providers report no durations, so their only rate was whole-call wall time presented identically to Ollama's authoritative figure; the fallback is now tagged via a new `evalTpsEstimated` field on `SseTokenStats` and rendered with an `(est)` suffix. (5) The status bar fell back to `promptTps` (prompt-processing speed, routinely 5-50x generation speed) when `evalTps` was absent - e.g. showing "3120 t/s" after an empty terminal chunk; the fallback chain (now `resolveTpsDisplay` in `src/app/lib/tpsDisplay.ts`) never displays the prompt rate. (6) A stream ending without `done` (user abort, error) left the PREVIOUS turn's `evalTps` in the merged tokenStats and displayed it as the aborted turn's rate: the server now ALWAYS includes `promptTps`/`evalTps` (null when unknown) in `done.tokenStats` so the merge clears stale values, and the client clears both the live value and the stored rate fields on error/abort paths - scoped to the owning session (`clearTpsReadout`), fixing a family of unscoped `SET_CURRENT_TPS` clears that wiped the VISIBLE session's badge when a background stream ended. Additionally: a background session's buffered `done` (the only carrier of its end-of-turn tokenStats) was discarded at stream end without ever being applied, freezing a stale mid-stream estimate on that session's slot forever - the `finally` blocks now call `flushBufferedDoneFor(ownerId)` to apply it before discarding the buffer; the route captures metrics only from non-heartbeat terminal chunks and via a guarded `extractTurnStats` (numeric strings coerced, non-finite dropped) so a malformed provider can neither zero the metrics nor string-concatenate token counts; `response.incomplete`/`failed`-style usage-less terminals now degrade to nulls instead of zeros. Sub-agent t/s continues to be displayed as before (per user instruction), now computed by the same shared `LiveThroughputMeter` (rebaseline + freeze semantics, 800 ms interval from `TPS_STATUS_MIN_INTERVAL_MS`) and still forwarded via `reportTps`. Two latent bugs also fixed en route: the `done`-case `SET_CURRENT_TPS` clear is now session-scoped like its siblings, and the pre-existing negated-ternary in the subagent_chunk owner lookup was inverted to satisfy lint. CLAUDE.md's "Live Token Meter" bullet was corrected (it referenced a non-existent `statusLine.ts`) and now documents the real surface and semantics.
  - Intent: The t/s badge should measure generation speed, stay stable during heavy tool-call generation, describe the whole turn, be honest about wall-clock estimates, and never resurrect a stale rate from a previous turn or another session - with the policy in one shared, tested place instead of hand-rolled locals in three files.
  - Covered by 46 tests in `scripts/test-token-throughput.mjs` (run with `npx tsx scripts/test-token-throughput.mjs`); compact-anchor, empty-response-recovery, subagent-MCP, and ollama-api-key suites unchanged and passing.

---

- 2026-09-07: Shared the empty-response recovery policy with sub-agents
  - Files: `src/services/emptyResponseRecovery.ts` (new), `src/app/api/chat/route.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/lib/debugLogger.ts`, `scripts/test-empty-response-recovery.mjs` (new), `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: Sub-agents silently ended whenever the model returned an empty response (zero streamed chunks, or only a bare channel label): the sub-agent loop pushed the empty message and broke with no content check, so the orchestrator received `[Sub-agent completed without a final text response.]` and the agent's log bubble rendered blank. The main chat loop already handled this with an empty-response retry (up to 3 synthetic user-role nudges), but that policy was private to `route.ts`. This change extracts the policy into a shared module `src/services/emptyResponseRecovery.ts`: `MAX_EMPTY_RESPONSE_RECOVERY_ATTEMPTS`, `CHANNEL_LABEL_ONLY_PATTERN`, `sanitizeAssistantTextFragment`, and `hasMeaningfulAssistantContent` moved verbatim from the route; a new `buildEmptyResponseRecoveryNudge()` builds the marker-prefixed `[System notice: Your last response was empty...]` nudge (identical wording for both loops, since sub-agents have the same nested tool access); and an `EmptyResponseRecoveryTracker` class owns the attempt counter with the established reset rules (meaningful reply or tool-call turn resets). The route now consumes the module (pure refactor, zero behaviour change: same constants, same nudge text, same non-persistence of the nudge, same token-anchor invalidation). The sub-agent loop's no-tool-calls branch gained the three-way decision: meaningful reply finishes as before; an empty reply while the tracker allows recovery pushes the empty assistant message plus the shared nudge into the agent's message history and continues the loop (with a visible `[empty response #N/3] retrying...` line in the agent's log bubble and a new `empty_response_recovery` phase in the diagnostic trace, added to `debugLogger.ts`); once the cap is exhausted the empty reply is accepted as final exactly as before. Side benefit: sub-agents never inspected `done_reason`, so an Ollama load/unload heartbeat terminal chunk used to end them silently with no content - the retry loop now naturally waits out the model load. As a deliberate boundary, only the recovery decision is shared; each loop keeps its own recovery action inline (persistence, anchors, and sinks differ between them). Covered by 23 tests in `scripts/test-empty-response-recovery.mjs` (run with `npx tsx scripts/test-empty-response-recovery.mjs`); compaction anchor and sub-agent MCP suites unchanged and passing.
  - Intent: An empty model response should never silently kill a sub-agent run; both agent surfaces now share one tested recovery policy instead of two drifting copies.

---

- 2026-09-03: Stopped rendering the empty assistant tool-call carrier bubble
  - Files: `src/components/ChatMessageBubble/ChatMessageBubble.tsx`, `docs/CHANGELOG.md`
  - Summary: After the subagent_log persistence fix, a blank rounded box appeared between the sub-agent's log bubble and its summary. It was the assistant message that carried the `run_subagents` tool call â€” `content=''` but `tool_calls` populated â€” and `ChatMessageBubble`'s empty-content branch rendered it as an "..." placeholder. The component now returns null for an assistant message with no content, no thinking, but at least one tool call; the following tool-result bubble already renders the call's output.
  - Intent: Remove the empty visual gap between a sub-agent's log and its result without losing any information.

- 2026-09-03: Fixed sub-agent log bubbles vanishing when the turn completes
  - Files: `src/app/api/chat/route.ts`, `docs/CHANGELOG.md`
  - Summary: The per-agent collapsible log bubbles (`subagent_log` messages) disappeared the moment a turn finished, leaving only the terse `run_subagents` tool result. Root cause was a two-part interaction. (1) Commit 4092111 (2026-08-12, "make prompt deletion reliable") added an end-of-turn `loadSessionMessages` reload in the client stream's `finally` block, which replaces the in-memory message list with the DB state after every completed stream. (2) The server never persisted `subagent_log` rows: the `subagentOutputSink` forwarded every line/chunk to the client via SSE but accumulated nothing, so the bubbles existed only in client memory during the turn. The database confirmed it â€” zero `subagent_log` rows across all sessions ever. The reload therefore wiped the bubbles before the next turn's `mergeClientMessages` could ever carry them into a write. Fix: the sink now accumulates per-agent content in a `subagentLogContent` map, and the tool-loop push site persists one `subagent_log` row per agentId into `pendingAppends` alongside the `run_subagents` assistant/tool-result block (rows go only to persistence â€” they are NOT pushed into `currentMessages`, which is the LLM-bound array). The abort path also flushes remaining rows so an interrupted batch keeps its bubbles. Rows carry numeric DB ids once persisted, so the next turn's `mergeClientMessages` dedupes them correctly.
  - Intent: Keep the sub-agent streaming UI (what each agent said and did) visible after the turn ends, instead of collapsing to just the final summary.

---

- 2026-09-03: Fixed namespaced MCP dispatch and the sub-agent MCP approval flow
  - Files: `src/tools/tools.ts`, `src/tools/impl/mcpTool.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/api/chat/route.ts`, `src/mcp/index.ts`, `scripts/test-subagent-mcp.mjs`
  - Summary: Two confirmed bugs fixed. (1) Direct `mcp__<server>__<tool>` tool calls â€” the form the merged tool list advertises to the LLM â€” returned `[Unknown tool: ...]` from both the main loop and sub-agents, because both dispatch sites resolved through the static native-tool registry and no code path dispatched a raw namespaced name. A new `tryRunNamespacedMCPCall` in `mcpTool.ts` now handles parseable namespaced names in both miss-branches (`handleToolCall` and `executeNestedToolCall`), reusing `runMCPCall`'s full check chain (blocklist, skill allowlist, YOLO, pre-approval ledger) with the dispatcher's approval gate as the fail-closed backstop. The main loop's approval gate was extended to cover direct namespaced calls, so non-YOLO calls now prompt instead of dead-ending with `[MCP call requires approval]`. (2) The sub-agent `mcp_call` approval gate prompted on every call and then discarded the user's approval: the immediate namespaced target was never recorded in the sub-agent's ledger, so the dispatcher rejected the very call the user had just approved. The gate now skips the prompt for ledger hits and `autoApprove` matches, records the approved target (and verified `grantedTools`) on approval, and mirrors the route's semantics via two shared helpers â€” `isAutoApprovedMCPTarget` (dispatcher-parity glob matching, fixing the route's exact-`includes` drift that made `autoApprove: ["list_*"]` configs prompt anyway) and `filterGrantedMCPTools` (grantedTools entries from unconfigured servers can no longer enter either ledger). `executeNestedToolCall` also gained the route's malformed-JSON-arguments guard. Covered by 17 tests in `scripts/test-subagent-mcp.mjs` (run with `npx tsx scripts/test-subagent-mcp.mjs`).
  - Intent: Make the documented MCP invocation paths actually work. The Phase 3.4 commit claimed direct namespaced calls worked end-to-end; in practice every direct call failed, and every sub-agent MCP approval was collected from the user and then thrown away.

---

- 2026-09-02: Made the LLM aware of compaction cycles since the last user prompt
  - Files: `src/app/api/chat/route.ts`, `src/tools/impl/subAgentTool.ts`, `src/services/compact/constants.ts`, `src/services/compact/index.ts`, `src/services/compact/split.ts`, `scripts/test-compact-anchor.mjs`, `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: Auto-compaction is no longer invisible to the model. The main chat loop keeps a per-request `compactionsSinceLastPrompt` counter (each real user message starts a fresh request, so it resets naturally per turn) and the existing LLM-only post-compaction nudge now reports the cumulative count plus current context-usage percentage: from the second compaction onward it adds an adaptive directive to economise on tool output, avoid re-reading large files or repeating searches, and reconsider the current approach. Sub-agents track their own local compaction count in the same way via the notice pushed by `autoCompactSubAgentIfNeeded`; the parent's count is deliberately not inherited (a threaded snapshot would go stale, and a sub-agent cannot change parent behaviour). Manual `/compact` is unchanged.
  - Intent: When the model repeatedly hits the context window limit and triggers auto-compaction within one turn, it can now recognise the pattern and adapt â€” tighter tool output, different strategy â€” instead of thrashing silently.

---

- 2026-09-02: Fixed the compaction anchor latching onto synthetic nudges
  - Files: `src/services/compact/split.ts`, `src/services/compact/constants.ts`, `src/services/compact/index.ts`, `src/app/api/chat/route.ts`, `src/tools/impl/subAgentTool.ts`, `scripts/test-compact-anchor.mjs`
  - Summary: `findLatestUserMessageIndex` in the compaction pipeline matched ANY `role === 'user'` message. The LLM-only post-compaction nudge is role `'user'` and lives in the in-memory history, so on the SECOND auto-compaction within one turn the "latest user message" verbatim-preservation anchor latched onto that nudge and the real user prompt was silently summarised away â€” breaking the documented guarantee that the most recent user prompt is always preserved verbatim. (The sub-agent path already compensated by force-restoring the orchestrator prompt; the main chat route did not.) Synthetic server-generated nudges (post-compaction notice, empty-response recovery, prompt-loop continuation) now all carry a `SYNTHETIC_NUDGE_MARKER` content prefix plus a `[End system notice]` suffix, and `isSyntheticNudge` excludes marker-prefixed messages from the anchor. Covered by 12 unit tests in `scripts/test-compact-anchor.mjs` (run with `npx tsx scripts/test-compact-anchor.mjs`).
  - Intent: Guarantee the real user prompt survives every compaction pass, including repeated auto-compactions within a single turn.

---

- 2026-09-01: Mid-turn model switching (main model, sub-agents, and compaction model)
  - Files: `src/app/lib/modelSwitchRegistry.ts`, `src/app/lib/switchModelClient.ts`, `src/app/api/chat/switch-model/route.ts`, `src/app/api/chat/route.ts`, `src/tools/toolRegistry.ts`, `src/tools/impl/subAgentTool.ts`, `src/app/lib/chatStore.ts`, `src/app/lib/debugLogger.ts`, `src/app/hooks/useChatStream.ts`, `src/components/ModelSelector/ModelSelector.tsx`, `src/components/SettingsModal/SettingsModal.tsx`, `src/components/StatusBar/StatusBar.tsx`, `src/components/StatusBar/StatusBar.scss`, `src/services/sseNames.ts`, `src/types/sse.ts`, `CLAUDE.md`, `docs/CHANGELOG.md`
  - Summary: The model of a turn that is already streaming can now be swapped without restarting it. `POST /api/chat/switch-model` writes a pending switch into a per-session in-memory registry (same singleton shape as `approvalRegistry.ts`); the running chat route consumes it at the top of each tool-call loop iteration and at the top of each sub-agent iteration, so the swap lands at a natural boundary rather than interrupting an in-flight LLM call. `const model` in the chat route became `let`, and the model-dependent setup (numCtx cap, vision support, sampling-parameter support, compaction runtime) was factored into reusable closures so the switch re-derives all of it for the new model instead of carrying stale verdicts forward. `requestContext.subAgent` is mutated in place and a new `SubAgentConfig.refreshModels` hook â€” mirroring the existing `approvalRequester` closure â€” lets a sub-agent apply the switch itself, which matters because a `run_subagents` batch parks the main loop for its entire duration. Main and compaction models can be switched independently; "Same as main" compaction follows a main-model switch automatically. The server confirms with a `status` event of phase `model_switched`, and the status bar shows "(switchingâ€¦)" until it arrives instead of flipping optimistically on click.
  - Intent: Let a user rescue a long, expensive turn â€” e.g. a multi-hour sub-agent investigation stalling on a weak or slow model â€” by moving it onto a stronger model in place, rather than aborting and losing the work.
  - Verified end-to-end against a live Ollama instance: main-loop switch applied at the next tool boundary; a switch fired 2.3s into a 15s `run_subagents` batch applied at 4.0s (server diagnostics confirm agent `a1` swapped models between its own iterations, and agent `a2` ran entirely on the new model); compaction-only switch changed the compaction model while leaving the main model untouched.
  - Note: an in-flight turn is bounded by the longest single tool call â€” a switch requested during a long non-sub-agent tool lands when that tool returns. The client's `retryPayloadRef` still replays a turn's original body, so a retry re-sends the pre-switch model; that is pre-existing retry behaviour and was left unchanged.
...
- 2026-08-31: Resolved all npm audit findings (14: 10 high, 4 moderate)
  ...
  - Summary: `npm audit fix` bumped `next` (16.2.9â†’16.3.3), `mermaid`, `dompurify`, `sharp`, `undici`, `hono`, `ip-address`, `js-yaml`, `nanoid`, `brace-expansion`, and `@hono/node-server` within their existing `package.json` ranges. The `overrides` pins for `postcss` (8.5.10) and `fast-uri` (3.1.2) â€” added in prior security fixes (see `ba994fe`, `68b2e4e`) â€” had themselves gone stale against newer advisories and were blocking `npm audit fix` from reaching them; bumped to `postcss@8.5.26` and `fast-uri@3.1.6`. `npm audit` now reports 0 vulnerabilities. Verified with `tsc --noEmit`, `npm run build`, and `npm test`.
  ...
  - Lesson: A version-pinning `overrides` entry fixes a vulnerability only at the moment it's added â€” it also freezes that package out of future `npm audit fix` runs. Revisit override pins whenever running an audit, not just when first adding them.
...
captures intent, the files touched, and a short rationale â€” use git
...
- Don't include the diff itself â€” that's what `git log -p` is for.
...
- 2026-08-02: Added transient-error retry layer inside the openai-compatible adapter; classifier now recognises `OpenAI.APIError`
  - Summary: The openai-compatible adapter now wraps the initial `client.responses.create` call in `withRetryAround` with exponential backoff and `Retry-After` (header or HTTP-date) support. Retry covers 408/409/429/500/502/503/504 by default and is pre-stream only â€” once any chunk has yielded, downstream errors propagate (the chat route's existing retry loop remains the safety net for that case). Defaults: `enabled=true`, `maxAttempts=3`, `baseDelayMs=1000`, `maxDelayMs=16000`. All adapter consumers â€” main chat, sub-agents, compaction distill/measure/summarize, title generation, prompt-loop judge/critic â€” benefit automatically; previously only the chat route retried, and even there it only matched axios errors. The route's `isRetryableError` classifier in `src/app/api/chat/sseStream.ts` was duck-typed to recognise any object with a numeric `.status`, so `OpenAI.APIError` from the SDK is now classified correctly when the inner layer exhausts and the route falls back. A new `retry: { enabled?, maxAttempts?, baseDelayMs?, maxDelayMs?, retryableStatuses? }` config block follows the `webSearch` precedent (allowlist + per-field validation + shallow merge). The existing `logAdapter400` debug-dump helper now runs only after the final failure, not per retry attempt, so a 429 storm no longer fills the repo root with `debug_400_*.json` files.
  - Intent: Free-tier OpenRouter models (and similar gateways with per-minute rate caps) now transparently recover from 429s on the first attempt â€” the user no longer sees a hard failure for what is fundamentally a transient quota blip. Pushing retry into the adapter (rather than expanding it at the route layer) gives every LLM call the same resilience with one change, and a single source of truth for retry semantics.
...
- 2026-07-24: Fixed slow `/api/models` by caching OpenAI-compatible model lists and adding request timeouts
  ...
  - Summary: The `/api/models` route was taking ~2.6 minutes because `fetchOpenAICompatibleModelInfo` re-fetched `/v1/models` once per model, producing N+1 identical HTTP calls. Added a 5-minute TTL module-level cache keyed by `(baseUrl, apiKey)` to `fetchOpenAICompatibleModels`, matching the existing `capResolver.ts`/`visionCache.ts` cache patterns. Added `invalidateOpenAICompatibleModelsCache(baseUrl?, apiKey?)` and wired it into `PUT /api/config` alongside `invalidateCapCache`/`invalidateVisionCache` so provider/baseUrl/API-key changes clear stale model lists. Also added a dedicated `MODEL_LIST_TIMEOUT_MS` (60 s) constant to `src/constants.ts` and applied it to the OpenAI-compatible `/v1/models` fetch and to Ollama's `/api/tags` and `/api/show` calls â€” the shared 15 s web-search timeout was too aggressive for slow OpenAI-compatible upstreams (the request would time out before the cache could be populated). The 60 s window is a one-time cost per 5-minute cache window.

---

- 2026-07-21: Migrated OpenAI-compatible adapter to the official `openai` npm SDK (Responses API)
  - Files: `src/services/adapters/openaiCompatibleAdapter.ts`, `package.json`, `package-lock.json`, `OPENAI_COMPATIBILITY_MIGRATION.md`
  - Summary: Replaced the hand-built `/v1/chat/completions` transport (axios + manual SSE parsing) with the official `openai` npm package's Responses API (`client.responses.create()`). The adapter now constructs a per-request `OpenAI` client with `baseURL` and `apiKey` from `LlmRequestContext`, maps the app's `ChatMessage[]` to `ResponseInputItem[]` (using `EasyInputMessage` for user/assistant messages, `ResponseFunctionToolCall` for tool calls, and `FunctionCallOutput` for tool results), and maps `Response`/`ResponseStreamEvent` output back to the existing `ChatApiResponse` shape. Streaming uses the SDK's `Stream<ResponseStreamEvent>` with event-driven accumulation of text deltas, reasoning text, and tool call fragments. Model listing (`/v1/models`) still uses axios since the SDK doesn't expose that endpoint for custom providers. The `setApiKey`/`clearApiKey` no-op shims are preserved for legacy callers. All downstream code (`src/services/llm.ts`, routes, components) is unchanged.
  - Intent: Delegate the OpenAI wire protocol to the official SDK instead of reimplementing it, reducing maintenance burden and enabling future Responses API features (reasoning, structured outputs, built-in tools) without further transport rewrites.

- 2026-07-19: Skills now load from the user-profile directory; `create_skill` supports a `location` argument
  - Files: `src/services/skillManager.ts`, `src/tools/impl/createSkillTool.ts`, `src/tools/toolRegistry.ts`, `src/app/api/skills/route.ts`, `src/components/sidebar/SkillsTab.tsx`, `CLAUDE.md`
  - Summary: Formalized the existing `~/.locopilot/skills/` directory as a first-class "user-profile" skills layer (was previously called "personal"). The `Skill` interface now carries a `location: 'project' | 'user-profile'` field stamped at discovery time, and the merge order still gives project precedence on name collision. Two new exported helpers â€” `getProjectSkillsDir()` and `getUserSkillsDir()` â€” centralize the path logic; the user-profile helper honors a `LOCOPILOT_HOME` environment override so installs can relocate the user root without code changes. `os.homedir()` already returns the right value on both Linux (`$HOME`) and Windows (`%USERPROFILE%`), so no `process.platform` branching is needed; `path.join` + `path.sep` produce correct separators on both. The `create_skill` tool gained an optional `location` argument (`'project' | 'user-profile'`, default `'project'`) so the LLM can write a skill into the user-profile directory when the user wants it to follow them across projects. Unknown values are rejected with a clear error. The `/api/skills` response now includes a `location` field on every item, and the Skills tab footer hint documents both directories with a cross-platform note (`%USERPROFILE%\.locopilot\skills\` on Windows). `discoverSkills` dedupes if both layers resolve to the same absolute path (e.g. when cwd lives under the home dir).
  - Intent: The "user-profile" directory was already on disk and already loaded, but the code called it "personal", the API didn't surface where a skill came from, the LLM had no way to write into it, and the Windows resolution was undocumented. Naming and exposing it makes the layer discoverable for users, LLMs, and packaging/installer flows on both platforms without adding any new on-disk directory.
  - Lesson: The home-directory-based path pattern (`os.homedir()` + `.locopilot/<thing>`) is already established for MCP config and OAuth tokens â€” extending it to a new feature is the lowest-risk, most consistent way to land cross-platform user data. Stamping a `location` field on the `Skill` interface at discovery time means every downstream consumer (API, UI, future badges) reads from a single source of truth instead of recomputing the path; the alternative (compare `skill.path` to the three known roots) would have been brittle across symlinks and `LOCOPILOT_HOME` overrides.

- 2026-07-16: Per-compaction-provider routing for the compaction / title LLM calls
  - Files: `src/app/lib/chatStore.ts`, `src/app/hooks/useStableRefs.ts`, `src/app/hooks/useChatStream.ts`, `src/app/hooks/useSlashCommands.ts`, `src/components/ModelSelector/ModelSelector.tsx`, `src/app/api/chat/route.ts`, `src/app/api/title/route.ts`, `src/app/api/compact/route.ts`
  - Summary: Added a transient `compactionProviderId` (`string | null`, in-memory only â€” not persisted to `config.json`) so the user can pick a compaction model from a _different_ provider than the active chat model. The ModelSelector's compaction branch now tags the picked row with its `providerId` and dispatches `SET_CONFIG { compactionModel, compactionProviderId }` together; "Same as main model" passes `''` + `null` together to clear both. The active check for compaction rows matches on the picked model's `providerId`. Request bodies for `/api/chat`, `/api/compact`, and `/api/title` now carry `compactionProviderId`. The title and compact routes do a _second_ `resolveProviderRequestContext(config, body.compactionProviderId, effectiveCompactionModel)` and use the resulting context (and that provider's `numCtx` via `getProviderNumCtx`) for the compaction/title LLM call. When no `compactionProviderId` is supplied (legacy client or "Same as main"), the route falls back to the main resolved context so today's behavior is preserved. The chat route's server-side auto-compact path applies the same pattern. Existing `resolveProvider` semantics from `45b2da3` carry through: a stale compactionProviderId returns `null` and the route falls back to the main context â€” no cross-wiring.
  - Intent: Enable setups like main = local Ollama and compaction = NVIDIA NIM (or any other cross-provider combination). The right provider is re-derived automatically from the picked model's `providerId` each time the user re-selects a compaction model in the grouped ModelSelector â€” no Settings/UI change required.
  - Lesson: Two-request-context routes (main + compaction) need a per-call resolver pair; carrying only `compactionModel` (the model name) without its provider left the route guessing via `provider.model` name-match, which only worked for the provider's default model and silently cross-wired for any other model on the same provider.
...
- 2026-07-16: Hardened multi-provider resolution, numCtx, and mount-time provider selection
  ...
  - Lesson: When a resolver accepts both an explicit id and a model name, an explicit-but-stale id must not silently degrade to "the first provider"; model-name recovery plus a `null` return is the safe fallback. Loader sequencing and ref-gated auto-selects are brittle on mount â€” a state-keyed reconciler effect is the robust pattern.
...
- 2026-07-05: Fixed model cap discovery to respect user's numCtx (Ollama auto-resize)
  ...
  - Summary: Three changes that together fix a class of bug where the user's `requestedNumCtx` was silently clamped to a much smaller value.
    (1) **Static probe is now authoritative** for the model cap. `getModelContextLimitFromInfo` scans `info.parameters` / `info.modelfile` for `num_ctx N` FIRST (capturing RoPE-scaled Modelfile overrides), then falls back to the structured walk over `model_info.<arch>.context_length`. The runtime probe (`/api/ps`) is now telemetry-only â€” the runner's transient KV-cache state is no longer treated as the cap. The cap resolver runs the static probe first; the runtime probe only matters when the static probe is unavailable.
    (2) **Ollama auto-resize**: Ollama's scheduler evicts the old runner and starts a new one with the requested `num_ctx` when a chat request asks for a larger context than the runner is currently allocated. So Locopilot no longer needs to "warn the user to restart Ollama" â€” sending the user's `requestedNumCtx` in the request body is sufficient. The user's 1M request is honored (clamped to the model's true max, e.g. 262144 for a Qwen3 35B base GGUF) without any manual `--num-ctx` flag.
...
- 2026-06-18: Fixed three compaction status-update bugs
  ...
  - Bug 3 (compaction phases leak across streams/sessions): Updated the `STOP_STREAMING` reducer case in `chatStore.ts` to clear `compactingPhases` when the visible session's stream ends. Guarded with `isVisibleSession` check to avoid clearing phases during the `session_created` placeholder migration (-1 â†’ realId) or when a non-current background session stops. Removed the dead `SET_STREAMING` action type, reducer case, and the orphaned `isStreaming` field from `ChatState` / initial state â€” these were never dispatched and were leftover from the CLI-to-web-UI migration.
...
- 2026-06-02: Added "Copy markdown" action button to assistant chat bubbles
  ...
  - Summary: Added a small hover-revealed button in the top-right corner of every assistant (`role: 'assistant'`) message bubble. Clicking it copies the bubble's raw markdown source (`message.content` â€” the verbatim streamed text held in the chat store) to the clipboard, using the same `navigator.clipboard.writeText` + `document.execCommand('copy')` fallback pattern already used by the per-`<pre>` code-block copy button in `MarkdownMessage.tsx`. The button label flips to `Copied!` with an inline checkmark SVG for 1500 ms after a successful copy, then reverts. The button is rendered in the React tree (not via DOM mutation), disabled while the bubble has no content, and uses the existing `--code-copy-bg` / `--code-copy-hover-bg` / `--code-copy-active-bg` / `--accent` / `--text-secondary` / `--glass-border-soft` variables â€” so it picks up both light and dark themes for free. The hover-reveal uses `opacity: 0 â†’ 0.85` on `.bubble-ai-wrap:hover` and `.bubble-ai-wrap:focus-within` so keyboard users get the same affordance. The `.bubble-ai-wrap` was given `position: relative` (one-line SCSS change) to anchor the absolutely-positioned button. A cleanup `useEffect` clears the `Copied!` timeout if the bubble unmounts mid-flash.
...
- 2026-04-14: Hardened compaction against massively oversized context windows (three-pronged fix)
  ...
  - Summary: Three independent fixes applied together. (1) **Distill preserved messages** â€” `distillToolMessages` is now also run on `historySplit.preservedRecentMessages` before those messages are assembled into `newMessages`, so large web-search and command-output tool results in the preserved window are compressed identically to those in the summarized window. This was the primary cause of 273 k â†’ 272 k "reduction" when 6 preserved tool messages each held 25 k+ chars. (2) **Bounded multi-pass retry** â€” the aggressive retry loop was generalized with a `remainingRetries` counter (default 2), replacing the single-shot `aggressiveFactor <= 1.0` guard, so the service can run up to 3 progressively more aggressive passes without possibility of infinite recursion. (3) **Caller-side over-budget warning** â€” both `autoCompactIfNeeded` and `COMPACT_HANDLER` now print a red warning if `result.stats.newTokenCount > numCtx` after all passes, making the failure visible instead of silently allowing the next turn to receive a 400 from Ollama.
...
- 2026-04-14: Emit thought summary immediately when tool calls arrive after thinking
  ...
  - Intent: Ensure users always see `(Thought for Xs Â· N chars)` the moment thinking transitions to tool-calling, not deferred until the end of the stream.
...
- 2026-04-04: Added automatic context compaction
  - Files: `index.ts`, `.github/copilot-instructions.md`
  - Summary: Added `autoCompactIfNeeded()` in `index.ts` that triggers compaction when estimated token usage reaches `AUTO_COMPACT_THRESHOLD_PCT` (92%). The function is called at the top of every tool-call loop iteration, covering both mid-turn (between tool calls) and end-of-turn growth. A yellow `âš¡` status line informs the user; errors are caught and shown as non-fatal warnings so the loop always continues.
...
(End of historical change log entries â€” entries above were preserved verbatim from the previous in-`CLAUDE.md` Change History section.)
...
- 2026-05-02: Removed global mutable state from toolRegistry.ts for multi-WebUI concurrency
  ...
  - Summary: Replaced three module-level `let` globals (`isYoloMode`, `webSearchSettings`, `subAgentConfig`) with a per-request `RequestContext` interface that is threaded through `handleToolCall()` â†’ `IToolCommand.execute()` â†’ tool implementations. Removed all setter/getter functions (`setYoloMode`, `setWebSearchConfig`, `setSubAgentConfig`, `isYolo`, `getSubAgentConfig`). The web SSE route creates a fresh `RequestContext` per HTTP request and passes it to every tool call, eliminating cross-talk between concurrent requests. The CLI path supplies context from the live config/session state. Also added a per-session write queue in `route.ts` that serializes synchronous `updateSessionMessages()` calls so concurrent requests to the same session do not overwrite each other.
