...
- 2026-09-02: Made the LLM aware of compaction cycles since the last user prompt
  ...
  - Intent: When the model repeatedly hits the context window limit and triggers auto-compaction within one turn, it can now recognise the pattern and adapt — tighter tool output, different strategy — instead of thrashing silently.
...
- 2026-07-19: Skills now load from the user-profile directory; `create_skill` supports a `location` argument
  ...
  - Lesson: The home-directory-based path pattern (`os.homedir()` + `.locopilot/<thing>`) is already established for MCP config and OAuth tokens — extending it to a new feature is the lowest-risk, most consistent way to land cross-platform user data. Stamping a `location` field on the `Skill` interface at discovery time means every downstream consumer (API, UI, future badges) reads from a single source of truth instead of recomputing the path; the alternative (compare `skill.path` to the three known roots) would have been brittle across symlinks and `LOCOPILOT_HOME` overrides.
