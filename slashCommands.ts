...
import { getToolUseNudge } from './tools/tools';
...
import type { ChatContext } from './types/chatConfig';
 ...
 */
export function replaceMessages<T>(target: T[], newMessages: T[]): void {
    target.length = 0;
...
type CommandResult = boolean | 'break';

// --- Command Handlers ---

const HELP_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
...
const MODEL_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    ...
    latestModels.forEach((m, i) => console.log(`  ${i + 1}. ${m}`));

    const selectedModel = await withExitGuard(async () => {
        return await select({
            message: 'Select a model to chat with:',
            choices: latestModels.map((m) => ({ name: m, value: m })),
...
const COMPACT_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    ...
    try {
        ...
        const result = await compactHistory(
            ...
            ctx.messages,
            ctx.numCtx,
            (status: string) => ctx.refreshTokenStatus(status, undefined, 'estimated', compactionModel),
        );
...
const DUMP_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    const currentSession = listSessions().find((session) => session.id === ctx.currentSessionId);
    try {
...
const SESSIONS_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
...
const DELETE_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    const sessions = listSessions();
    if (sessions.length === 0) {
    ...
    if (toDelete !== null) {
        const target = sessions.find((s) => s.id === toDelete);
        ...
        if (toDelete === ctx.currentSessionId) {
            ...
            const freshMessages = [{ role: 'system' as const, content: ctx.systemPrompt }];
...
const NUDGE_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    ctx.messages.push({ role: 'user', content: getToolUseNudge(false) });
    console.log(chalk.dim('\n[Manual nudge sent to AI...]\n'));
    return false; // Continue with AI generation loop
};

const EXIT_HANDLER = async (): Promise<'break'> => 'break';

const NEW_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
...
const SETTINGS_HANDLER = async (ctx: ChatContext): Promise<CommandResult> => {
    ...
    const action = await withExitGuard(async () => {
        return await select({
            ...
            choices: [
                {
                    name: `Execution Mode (${ctx.config.yolo ? 'YOLO' : 'Standard'})`,
                    value: 'mode',
                },
                {
                    name: `Thinking (${ctx.config.thinkingEnabled !== false ? chalk.green('Enabled') : chalk.red('Disabled')})`,
                    value: 'thinking',
                },
                {
                    name: `Context Length (${ctx.config.numCtx ?? DEFAULT_NUM_CTX})`,
                    value: 'num_ctx',
                },
                {
                    name: `Chat Timeout (${(ctx.config.chatTimeoutMs ?? DEFAULT_OLLAMA_CHAT_TIMEOUT_MS) / 1000}s)`,
                    value: 'chat_timeout',
                },
                {
                    name: `Compaction Model (${effectiveCompactionModel})`,
                    value: 'compaction_model',
                },
                {
                    name: `Web Search: Max Queries (${ctx.config.webSearch?.maxQueries ?? DEFAULT_WEB_SEARCH_MAX_QUERIES})`,
                    value: 'web_max_queries',
                },
                {
                    name: `Web Search: Results Per Query (${ctx.config.webSearch?.resultsPerQuery ?? DEFAULT_WEB_SEARCH_RESULTS_PER_QUERY})`,
                    value: 'web_results_per_query',
                },
                {
                    name: `Web Search: Page Char Limit (0 = unlimited) (${ctx.config.webSearch?.perPageCharLimit ?? DEFAULT_WEB_SEARCH_PER_PAGE_CHAR_LIMIT})`,
                    value: 'web_per_page_char_limit',
                },
    ...
    if (action === 'mode') {
        const mode = await withExitGuard(async () => {
            return await select({
                ...
                choices: [
                    ...
                    {
                        name: chalk.red.bold('YOLO') + '     (Automatic command execution - USE WITH CAUTION)',
                        value: 'yolo',
                    },
                ],
        ...
        const timeoutInput = await withExitGuard(async () => {
            return await input({
                ...
                default: String(
                    (ctx.config.chatTimeoutMs ?? DEFAULT_OLLAMA_CHAT_TIMEOUT_MS) / 1000,
                ),
                validate: (value: string): boolean | string => {
        ...
        if (inputVal !== null) {
            ...
            const newWebSearch = {
                maxQueries: parsed,
                resultsPerQuery:
                    ctx.config.webSearch?.resultsPerQuery ?? DEFAULT_WEB_SEARCH_RESULTS_PER_QUERY,
                perPageCharLimit:
                    ctx.config.webSearch?.perPageCharLimit ?? DEFAULT_WEB_SEARCH_PER_PAGE_CHAR_LIMIT,
            };
        ...
        const inputVal = await withExitGuard(async () => {
            return await input({
                ...
                default: String(
                    ctx.config.webSearch?.perPageCharLimit ??
                        DEFAULT_WEB_SEARCH_PER_PAGE_CHAR_LIMIT,
                ),
                validate: (value: string): boolean | string => {
                    const parsed = parseNonNegativeInteger(Number.parseInt(value, 10));
                    return parsed !== null ? true : 'Please enter a non-negative integer.';
                },
            });
        });
        if (inputVal !== null) {
            ...
            if (parsed !== null) {
                const newWebSearch = {
                    maxQueries:
                        ctx.config.webSearch?.maxQueries ?? DEFAULT_WEB_SEARCH_MAX_QUERIES,
                    resultsPerQuery:
                        ctx.config.webSearch?.resultsPerQuery ??
                        DEFAULT_WEB_SEARCH_RESULTS_PER_QUERY,
...
export const SLASH_COMMANDS = [
    ...
    {
        name: chalk.blue('/compact') + '  - Summarise conversation history to save context',
        value: '/compact',
    },
    {
        name: chalk.blue('/dump') + '     - Export the current conversation as a markdown debug file',
        value: '/dump',
    },
    {
        name: chalk.blue('/sessions') + ' - List and switch to a previous conversation',
        value: '/sessions',
    },
...
export const COMMAND_HANDLERS: Record<string, (ctx: ChatContext) => Promise<CommandResult>> = {
    '/new': NEW_HANDLER,
    '/model': MODEL_HANDLER,
    ...
    '/nudge': NUDGE_HANDLER,
    '/exit': EXIT_HANDLER as unknown as (ctx: ChatContext) => Promise<CommandResult>,
    '/help': HELP_HANDLER,
};
